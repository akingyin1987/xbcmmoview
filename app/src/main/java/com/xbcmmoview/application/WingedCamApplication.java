...
public class WingedCamApplication extends Application {
    ...
    public static void setFlipStatus(int flipStatus) {
        WingedCamApplication.flipStatus = flipStatus;
    }
    ...
    public static void setLight_state(String light_state) {
        WingedCamApplication.light_state = light_state;
    }
    ...
    private void initImageLoader(Context context) {
        ImageLoader.getInstance().init(new Builder(context).memoryCacheExtraOptions(480, 800).threadPoolSize(3).threadPriority(3).denyCacheImageMultipleSizesInMemory().diskCacheFileNameGenerator(new Md5FileNameGenerator()).memoryCache(new UsingFreqLimitedMemoryCache(2097152)).memoryCacheSize(2097152).diskCacheSize(52428800).tasksProcessingOrder(QueueProcessingType.LIFO).diskCache(new UnlimitedDiskCache(StorageUtils.getOwnCacheDirectory(context, "imageLoader/Cache"))).imageDownloader(new BaseImageDownloader(context, BaseImageDownloader.DEFAULT_HTTP_CONNECT_TIMEOUT, 30000)).build());
    }
