    ...
    protected void onPostExecute(LoadTFRecordsTaskResult result) {
      ...
      if (result.error == ERROR.NO_ERROR) {
        ...
        try {
          ...
          tomorrow_zero.set(Calendar.HOUR_OF_DAY, 0);
          tomorrow_zero.set(Calendar.MINUTE, 0);
          tomorrow_zero.set(Calendar.SECOND, 0);
          tomorrow_zero.set(Calendar.MILLISECOND, 0);
          tomorrow_zero.add(Calendar.DATE, 1);
          ...
          for (int i = 0; i < records.length(); i++) {
            ...
            if (!record_t.before(IPCam.this.m_tf_record_info.days[0].zero) && record_t.before(tomorrow_zero)) {
              ...
              h = record_t.get(Calendar.HOUR_OF_DAY);
              q = record_t.get(Calendar.MINUTE) / 15;
